function renderPaperDiagram() {
  const diagram = document.getElementById("paperDiagram");
  if (!diagram) return;
  const nodes = [
    ["RefVideo-6M", "center"], ["Video Tasks", "top"],
    ["Global Editing", "left-top"], ["Local Editing", "right-top"],
    ["Controllable Editing", "left-bottom"], ["Image Tasks", "right-bottom"],
    ["Reference-guided", "bottom"],
  ];
  diagram.innerHTML = nodes.map(([label, position]) => `<div class="diagram-node ${position}">${label}</div>`).join("") +
    ["l1", "l2", "l3", "l4", "l5"].map((line) => `<i class="diagram-line ${line}" aria-hidden="true"></i>`).join("");
}
    ...
    ? new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            ...
            if (video._comparisonSync) {
              video._comparisonSync.setVisible(entry.isIntersecting);
        ...
        // Start only when a card is about to enter the viewport. A large margin
        // makes several multi-megabyte clips compete for bandwidth at once.
...
function createMediaPanel(label, source, kind, isResult = false) {
  ...
  if (kind === "video") {
    ...
    video.autoplay = true;
    video.preload = "none";
    video.poster = source.replace(/\.mp4$/i, "-poster.webp");
...
function attachSliderDrag(slider, autoPlay = false) {
  ...
  const setPosition = (percentage) => {
    ...
    videoAfter.style.clipPath = `polygon(0 0, ${clamped}% 0, ${clamped}% 100%, 0 100%)`;
  };
  ...
  const animate = (timestamp) => {
    ...
    setPosition(direction === 1 ? eased * 100 : 100 - eased * 100);
    if (progress >= 1) {
      direction *= -1;
  ...
  const startAnimation = () => {
    if (!autoPlay || animationFrame !== null) return;
    setPosition(0);
    direction = 1;
  ...
  const stopDrag = () => {
    ...
    isDragging = false;
    if (autoPlay) {
      // Let the manually selected position remain visible briefly before the
      // hands-free left-to-right animation starts again.
      clearTimeout(resumeTimer);
...
function createMediaElement(kind, src, fallbackLabel) {
  if (kind === "video") {
    ...
    v.autoplay = true;
    v.preload = "none";
    v.poster = src.replace(/\.mp4$/i, "-poster.webp");
    v.dataset.src = src;
...
function synchronizeComparisonVideos(slider) {
  const videos = [...slider.querySelectorAll("video")];
  if (videos.length !== 2) return;

  const source = slider.querySelector(".video-after video");
  const target = slider.querySelector(".video-before video");
  if (!source || !target) return;
  let visible = false;
  let ready = false;
  const playPair = () => {
    if (!visible || !ready) return;
    // Match the pair's wall-clock duration with playbackRate. Native video
    // playback stays smooth; repeatedly assigning currentTime causes stutter.
    source.loop = true;
    target.loop = true;
    source.playbackRate = 1;
    target.playbackRate = target.duration / source.duration;
    const sourcePlay = source.play();
    const targetPlay = target.play();
    [sourcePlay, targetPlay].forEach((playPromise) => {
      if (playPromise && typeof playPromise.catch === "function") playPromise.catch(() => {});
    });
  };

  const updateReady = () => {
    ready = [source, target].every(
  ...
  source.addEventListener("loadedmetadata", updateReady);
  target.addEventListener("loadedmetadata", updateReady);
...
function createComparisonSlider(sourceSrc, resultSrc, kind, aspectClass = "aspect-16x10") {
  ...
  if (kind === "video") synchronizeComparisonVideos(slider);

  // Both video and image comparisons gently demonstrate the edit automatically;
  // the handle remains manually draggable at any time.
  requestAnimationFrame(() => attachSliderDrag(slider, true));
...
function createCard(task) {
  ...
  refRow.innerHTML = `
    ...
    <div class="meta-box meta-prompt">
      ...
      <p class="meta-prompt-text" data-task-id="${task.id}">${task.prompt}</p>
    </div>
  `;
...
function releaseGalleryMedia() {
  galleryGrid.querySelectorAll("video").forEach((video) => {
    if (lazyVideoObserver) lazyVideoObserver.unobserve(video);
    video.pause();
...
function renderGallery() {
  // Filters replace the whole gallery. Detach old media first so removed video
  // elements cannot keep downloading or remain retained by the observer.
  releaseGalleryMedia();
...
const statisticsData = {
  videoTasks: [
    ["Relight", 1, "global"], ["Style Transfer", 6, "global"],
    ["Weather / Season", 2, "global"], ["Recam", 1, "global"],
    ["VFX", 1, "global"], ["Object Addition", 9, "local"],
    ["Object Removal", 5, "local"], ["Object Recolor", 8, "local"],
    ["Retexture", 7, "local"], ["Object Swap", 7, "local"],
    ["Background Swap", 1, "local"], ["Try-on", 6, "local"],
    ["Human Swap", 1, "local"], ["Watermark Removal", 3, "local"],
    ["Subtitle Removal", 4, "local"], ["Colorization", 3, "control"],
    ["Deblur", 3, "control"], ["Upscale", 3, "control"],
    ["Outpainting", 3, "control"], ["Inpainting", 3, "control"],
    ["HED to Video", 3, "control"], ["Depth to Video", 3, "control"],
    ["Canny to Video", 3, "control"], ["Scribble to Video", 3, "control"],
    ["Video to Maps", 3, "control"], ["Object Detection", 3, "control"],
  ],
  videoInstructions: {
    labels: ["0–9", "10–19", "20–29", "30–39", "40–49", "50–59", "60–69", "70–79", "80–89", "90–99"],
    values: [6, 11, 16, 19, 20, 13, 8, 4, 2, 1],
  },
  videoLength: { labels: ["81 Frames", "129 Frames"], values: [54, 50] },
  imageInstructions: {
    labels: ["0–2", "3–5", "6–8", "9–11", "12–14", "15–17", "18–20"],
    values: [40, 31, 13, 7, 4, 3, 2],
  },
  references: [
    { title: "Video Location / Direction", value: 7, color: "#29b6d8" },
    { title: "Video Appearance", value: 93, color: "#ffca3a" },
  ],
};

function renderStatisticsDashboard() {
  const root = document.getElementById("statisticsDashboard");
  if (!root) return;
  ...
  const taskTotal = statisticsData.videoTasks.reduce((sum, item) => sum + item[1], 0);
  const taskGradient = statisticsData.videoTasks.reduce(
    (state, item) => {
      const start = state.total;
      const end = start + (item[1] / taskTotal) * 100;
      state.stops.push(`${palette[item[2]]} ${start}% ${end}%`);
      state.total = end;
      return state;
    },
    { total: 0, stops: [] },
  ...
  const makeBars = ({ labels, values }, color, max) => `
    <div class="stats-bars" style="--bar-color:${color}">
      ${values.map((value, index) => `
        <div class="stats-bar-item" title="${labels[index]}: ${value}%">
          <span class="stats-bar-value">${value}%</span>
          <div class="stats-bar-track"><i style="height:${(value / max) * 100}%"></i></div>
          <span class="stats-bar-label">${labels[index]}</span>
        </div>`).join("")}
    </div>`;

  const itemColors = ["#ec5255", "#f7a02e", "#ffd335", "#b2db12", "#34cf50", "#7fdcd1", "#5cb3ed", "#8686e2", "#e9b9f0"];
  const groupColors = { global: "#ec5255", local: "#5cb3ed", control: "#ffd335" };
  const center = 350;
  const centerY = 270;
  const innerRadius = 82;
  const barRadius = 112;
  const barHeight = 82;
  const outerRadius = barRadius + barHeight;
  const totalItems = statisticsData.videoTasks.length;
  const polarPoint = (radius, angle) => ({ x: center + Math.cos(angle) * radius, y: centerY + Math.sin(angle) * radius });
  const arcPath = (radius, start, end) => {
    const a = polarPoint(radius, start), b = polarPoint(radius, end);
    return `M ${a.x} ${a.y} A ${radius} ${radius} 0 ${end - start > Math.PI ? 1 : 0} 1 ${b.x} ${b.y}`;
  };
  const taskSegments = statisticsData.videoTasks.map(([label], index) => {
    const angle = -Math.PI / 2 + (index + 0.5) * Math.PI * 2 / totalItems;
    const width = Math.PI * 2 / totalItems * .76;
    const points = [polarPoint(barRadius, angle - width / 2), polarPoint(barRadius, angle + width / 2), polarPoint(outerRadius, angle + width / 2), polarPoint(outerRadius, angle - width / 2)].map((p) => `${p.x},${p.y}`).join(" ");
    return `<polygon points="${points}" fill="${itemColors[index % itemColors.length]}" stroke="#fff" stroke-width="1"><title>${label}</title></polygon>`;
  }).join("");
  const groupRanges = [];
  let groupStart = 0;
  ["global", "local", "control"].forEach((group) => {
    const count = statisticsData.videoTasks.filter((item) => item[2] === group).length;
    groupRanges.push({ group, start: -Math.PI / 2 + groupStart * Math.PI * 2 / totalItems, end: -Math.PI / 2 + (groupStart + count) * Math.PI * 2 / totalItems });
    groupStart += count;
  });
  const groupArcs = groupRanges.map(({ group, start, end }) => `<path d="${arcPath(innerRadius, start, end)}" fill="none" stroke="${groupColors[group]}" stroke-width="9"/>`).join("");
  const taskLabels = statisticsData.videoTasks.map(([label, value], index) => {
    const radians = -Math.PI / 2 + (index + 0.5) * Math.PI * 2 / totalItems;
    const point = polarPoint(outerRadius + 26, radians);
    let rotation = radians * 180 / Math.PI + 90;
    if (rotation > 90 && rotation < 270) rotation += 180;
    const anchor = point.x < center - 8 ? "end" : point.x > center + 8 ? "start" : "middle";
    return `<g class="radial-label" tabindex="0"><title>${label}: ${value}%</title><text x="${point.x}" y="${point.y}" text-anchor="${anchor}" transform="rotate(${rotation} ${point.x} ${point.y})"><tspan class="radial-value">${value}%</tspan><tspan x="${point.x}" dy="13">${label}</tspan></text></g>`;
  }).join("");

  root.innerHTML = `
    <article class="stats-card stats-task-card">
      <div class="stats-card-heading">
        <div><span class="stats-kicker">RefVideo-6M Video Pairs Statistics</span><h4>Task Distribution</h4></div>
        <strong class="stats-total">${taskTotal}%</strong>
      </div>
      <div class="radial-chart" role="img" aria-label="Video task distribution radial chart">
        <svg viewBox="0 0 700 540" preserveAspectRatio="xMidYMid meet">
          <circle cx="350" cy="270" r="79" fill="#fff" stroke="#eef1f4" stroke-width="1"/>
          ${groupArcs}
          ${taskSegments}
          ${taskLabels}
          <text x="350" y="264" text-anchor="middle" class="radial-center">RefVideo-6M</text>
          <text x="350" y="283" text-anchor="middle" class="radial-center-sub">Video</text>
        </svg>
      </div>
      <div class="stats-group-legend">
        <span><i style="background:${palette.global}"></i>Global</span>
        <span><i style="background:${palette.local}"></i>Local</span>
        <span><i style="background:${palette.control}"></i>Control</span>
      </div>
    </article>

    <div class="stats-side-grid">
      <article class="stats-card">
        <div class="stats-card-heading"><div><span class="stats-kicker">Video</span><h4>Instruction Statistics</h4></div></div>
        ${makeBars(statisticsData.videoInstructions, "#556ee6", 20)}
      </article>
      <article class="stats-card">
        <div class="stats-card-heading"><div><span class="stats-kicker">Video</span><h4>Length Statistics</h4></div></div>
        ${makeBars(statisticsData.videoLength, "#ff6b81", 60)}
      </article>
      <article class="stats-card stats-reference-card">
        <div class="stats-card-heading"><div><span class="stats-kicker">Video</span><h4>Reference Statistics</h4></div></div>
        <div class="stats-reference-list">
          ${statisticsData.references.map((item) => `
            <div class="stats-reference-item">
              <div class="stats-mini-donut" style="--value:${item.value};--ring:${item.color}">
      ...
      <article class="stats-card">
        <div class="stats-card-heading"><div><span class="stats-kicker">Image pairs</span><h4>Instruction Statistics</h4></div></div>
        ${makeBars(statisticsData.imageInstructions, "#20bfa9", 45)}
      </article>
    </div>`;
}
...
hydratePrompts([...videoTasks, ...imageTasks]).then(() => {
  // Do not rebuild every card here: doing so discards active media and can
  // trigger a second round of video downloads. Update prompt text in place.
  allTasks.forEach((task) => {
    const prompt = galleryGrid.querySelector(
      `.meta-prompt-text[data-task-id="${task.id}"]`,
    );
